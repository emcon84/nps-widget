...
async function main() {
  ...
  try {
    // Create a test user for development (sin bcrypt por simplicidad)
    const user = await prisma.user.upsert({
      where: { email: "test@example.com" },
      update: {},
      create: {
        id: "1", // ID fijo para coincidir con NextAuth
        email: "test@example.com",
        name: "Test User",
        password: "123456", // Texto plano por ahora para simplificar
      },
    });
